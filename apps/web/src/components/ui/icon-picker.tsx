...
const COLLECTIONS: Collection[] = [
    { prefix: 'lucide', label: 'Lucide', load: () => import('@iconify-json/lucide/icons.json').then((m) => m.default as IconifyJSON) },
    { prefix: 'tabler', label: 'Tabler', load: () => import('@iconify-json/tabler/icons.json').then((m) => m.default as IconifyJSON) },
    { prefix: 'heroicons', label: 'Heroicons', load: () => import('@iconify-json/heroicons/icons.json').then((m) => m.default as IconifyJSON) },
    { prefix: 'material-symbols', label: 'Material', load: () => import('@iconify-json/material-symbols/icons.json').then((m) => m.default as IconifyJSON) },
    { prefix: 'mdi', label: 'MDI', load: () => import('@iconify-json/mdi/icons.json').then((m) => m.default as IconifyJSON) },
    { prefix: 'ph', label: 'Phosphor', load: () => import('@iconify-json/ph/icons.json').then((m) => m.default as IconifyJSON) },
    { prefix: 'ri', label: 'Remix', load: () => import('@iconify-json/ri/icons.json').then((m) => m.default as IconifyJSON) },
    { prefix: 'carbon', label: 'Carbon', load: () => import('@iconify-json/carbon/icons.json').then((m) => m.default as IconifyJSON) },
    { prefix: 'fa6-solid', label: 'FA Solid', load: () => import('@iconify-json/fa6-solid/icons.json').then((m) => m.default as IconifyJSON) },
    { prefix: 'bi', label: 'Bootstrap', load: () => import('@iconify-json/bi/icons.json').then((m) => m.default as IconifyJSON) },
    { prefix: 'ion', label: 'Ionicons', load: () => import('@iconify-json/ion/icons.json').then((m) => m.default as IconifyJSON) },
    { prefix: 'octicon', label: 'Octicons', load: () => import('@iconify-json/octicon/icons.json').then((m) => m.default as IconifyJSON) },
    { prefix: 'simple-icons', label: 'Brands', load: () => import('@iconify-json/simple-icons/icons.json').then((m) => m.default as IconifyJSON) },
    { prefix: 'twemoji', label: 'Twemoji', load: () => import('@iconify-json/twemoji/icons.json').then((m) => m.default as IconifyJSON) },
];
...
export function IconPicker({ open, onOpenChange, value, onChange, title = 'Choose icon' }: IconPickerProps) {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="flex h-[78vh] max-h-[720px] w-full max-w-3xl flex-col gap-3 p-0">
                ...
                <div className="flex gap-1 overflow-x-auto border-b px-3 pb-1 pt-0">
                    <TabButton
                        active={activePrefix === 'recent'}
                        onClick={() => setActivePrefix('recent')}
                    >
                    ...
                    <TabButton
                        active={activePrefix === 'emoji'}
                        onClick={() => setActivePrefix('emoji')}
                    >
                        Emoji
                    </TabButton>
                </div>
                ...
                {names.map((name) => {
                    ...
                    return (
                        <button
                            ...
                            title={name}
                            className={cn(
                                'grid aspect-square place-items-center rounded-md text-foreground/80 transition-colors hover:bg-accent hover:text-foreground',
                                isSelected && 'bg-primary/15 text-primary ring-1 ring-primary',
                            )}
                        >
            ...
            <div className="grid grid-cols-[repeat(auto-fill,minmax(44px,1fr))] gap-1">
                {common.map((e) => (
                    <button
                        ...
                        className="grid aspect-square place-items-center rounded-md text-xl hover:bg-accent"
                    >
                        {e}
                    </button>
                ))}

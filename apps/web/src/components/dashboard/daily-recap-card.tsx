...
export function DailyRecapCard() {
  const t = useTranslations('dashboard.dailyRecap');
  ...
  return (
    <div className="rounded-2xl border bg-emerald-500/5 p-4">
      <div className="text-muted-foreground mb-2 flex items-center gap-1.5 text-[11px] uppercase tracking-wide">
        ...
        <span className="text-muted-foreground/70 ml-auto">
          {t('summary', {
            notes:
              data.noteCount === 1
                ? t('noteCountOne', { count: data.noteCount })
                : t('noteCountOther', { count: data.noteCount }),
      ...
      {data.summary ? (
        ...
        <p className="text-muted-foreground text-sm">{t('noProvider')}</p>
      )}
      {data.summary && (
        <div className="mt-3">
          <button

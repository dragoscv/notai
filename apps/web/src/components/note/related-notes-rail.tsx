...
export function RelatedNotesRail({ noteId }: Props) {
  const t = useTranslations('editor.links.related');
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {items.map((it) => (
          <Link
            ...
            title={t('distanceTitle', { value: it.distance.toFixed(3) })}
          >
            <span className="size-3.5 text-center">
              {it.icon || <FileText className="size-3 opacity-60" />}
            </span>
            <span className="max-w-[14rem] truncate">{it.title || t('untitled')}</span>
          </Link>
        ))}
      </div>
    </div>

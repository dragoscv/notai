...
import { findActiveClientByClientId, findConsent, issueToken } from '@/server/oauth-store';
...
export default async function AuthorizePage({
  ...
  if (!params.code_challenge) {
    return errorPage('invalid_request', 'code_challenge is required (PKCE).');
  }
  ...
  if (grantedScopes.length === 0) {
    return errorPage('invalid_scope', 'None of the requested scopes are allowed for this client.');
  }
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-md flex-col justify-center p-8">
      <div className="bg-card rounded-2xl border p-6 shadow-sm">
        <h1 className="font-serif text-xl font-semibold tracking-tight">Authorization failed</h1>
        <p className="text-muted-foreground mt-1 text-xs uppercase tracking-wider">{code}</p>
        <p className="text-foreground/80 mt-3 text-sm">{msg}</p>
      </div>
    </main>
  );
}

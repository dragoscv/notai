...
const TOOLS: ToolDef[] = [
    {
        name: 'notes.list',
        description: 'List the user\'s notes (most recent first). Optional filters by folder/archived/favourite/pinned.',
    ...
    {
        name: 'notes.update',
        description: 'Update a note. Only the owner may update via the API. Pass only the fields you want to change.',
        requiredScope: 'notes:write',
        inputSchema: {
            type: 'object',
            required: ['id'],
...
function jsonRpcError(code: number, message: string, id: unknown = null) {
    return Response.json(
        ...
        { status: code === -32700 || code === -32600 ? 400 : 200, headers: { 'cache-control': 'no-store' } },
    );
}

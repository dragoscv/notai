...
import { db, and, desc, eq, isNull, oauthClients, oauthConsents, oauthTokens } from '@notai/db';
import {
  formatScopes,
  generateClientId,
...
export async function revokeConnectedApp(clientInternalId: string) {
  ...
  await db
    ...
    .where(and(eq(oauthConsents.userId, user.id), eq(oauthConsents.clientId, clientInternalId)));
  await db
    .update(oauthTokens)
    .set({ revokedAt: new Date() })
    .where(

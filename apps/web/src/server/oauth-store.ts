...
export async function consumeToken(id: string): Promise<void> {
  await db.update(oauthTokens).set({ consumedAt: new Date() }).where(eq(oauthTokens.id, id));
}
...
export async function findActiveClientByClientId(clientId: string): Promise<OauthClient | null> {
  const [row] = await db
    .select()
    .from(oauthClients)

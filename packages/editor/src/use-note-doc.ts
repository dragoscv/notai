...
if (
  ...
  !(window as unknown as { __notaiHpWarnPatched?: boolean }).__notaiHpWarnPatched
) {
  ...
  console.warn = function patched(...args: unknown[]) {
    ...
    if (
      typeof first === 'string' &&
      first.includes('[HocuspocusProvider]') &&
      first.includes('authentication token is required')
    ) {
      return;
    }
...
function acquire(key: string, params: { noteId: string; url: string; token: string }): CachedEntry {
